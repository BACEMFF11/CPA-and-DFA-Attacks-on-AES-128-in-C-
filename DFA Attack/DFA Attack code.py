...
# 📁 1️⃣ loading cts.npy and fcts.npy
cts = np.load("C:\\your\\ownPath\\ciphertexts.npy")  # correct ciphertexts
fcts = np.load("C:\\your\\ownPath\\faulty_ciphertexts.npy")  # faultyciphertexts

# 🛠️ 2️⃣ Verification of the coherence of the data 
...
# 4️⃣   Calculating the error 
def calculer_erreur(C, Cf, k):
    state_C = sbox_inv[(C ^ k) % 256]
    state_Cf = sbox_inv[(Cf ^ k) % 256]
    return state_C ^ state_Cf

# 5️⃣ Function for calculating Shannon entropy
def entropie(data):
    freq = Counter(data)
    total = sum(freq.values())
    return -sum((count / total) * math.log2(count / total) for count in freq.values() if count > 0)

# 📂 Creation of plots folder
output_dir = "D:\\entropy_plots"
os.makedirs(output_dir, exist_ok=True)

# Calculating errors and entropies for eah byte of the AES encryption key
cle_complete = []

for octet_idx in range(16):  # For each byte
    entropies_octet = []

    for k in range(256):  # Key Hypothisis
    ...
    # 🎨 Visualisation of entropy plots 
